...
The visual page shows whether the selected engine already has credentials. “获取模型列表” replaces the free-text model field with a full dropdown populated from provider-native endpoints; the dropdown includes an option to return to manual entry. “测试接口” checks the address and authentication without model generation. The terminal wizard provides the same discovery and test operations with numbered choices. A newly entered key is used for those requests and saved only at the end; otherwise the environment or saved credential is used. Neither interface displays a saved key, asks for a book, or creates a translation plan. The terminal mode deliberately rejects piped/non-TTY input and offers no key command-line flag.

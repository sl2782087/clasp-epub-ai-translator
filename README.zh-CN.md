...
- 高保真图片翻译：完整提取并检查封面与正文图片，逐图选方法，分别验收无字背景、排版、文字、保护区像素与 EPUB 回填结果。
...
需要可交付质量时选择 `--image-translation review`。正文完成后，包装器会生成名称明确的“文字版”中间 EPUB，以及包含全部原图、联系表、`inventory.json` 和强制 `decisions.json` 的 `image-review` 工作目录。

Codex 必须检查每张图（包括封面），记录 `translate / keep / uncertain`，并根据白底图、时刻表、地图、封面、复杂插画或可编辑 SVG 分别选择方法。修改过的位图需要独立通过文字、视觉和像素保护检查；封面还必须单独验收无字背景与排版层。打包器会拒绝未审阅或未通过检查的替换图，并验证扩展名、MIME、引用、正文可见文本、spine 和未修改图片字节。
...
书籍正文会发送给你选择的模型服务。高保真图片流程默认使用本地确定性工具；只有用户明确同意对某一步调用视觉或生成式编辑服务时，对应图片才会离开设备。实验性自动图片模式会把候选图片发送到配置的接口。项目本身没有中转服务器，也不会接收这些内容。详情见 [PRIVACY.md](PRIVACY.md)。
